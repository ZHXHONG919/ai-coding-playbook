...
- AI provider / 外部平台改动：在应用真实装配中替换外部边界验证；真实 provider 最小 case 只在本次验收要求且已授权时执行，分别记录证明范围。
...
- 普通任务按改变的行为、反例、真实入口和预期选择最小自测；通过后可以继续。
- 改变后续依赖的共享规则时先审该有限范围；完整功能批次独立验证与 CR 后才验收。
- 验证与 CR 固定同一代码及契约版本；主线程裁决问题，修复按影响域复核，同类再发先检查共同原因。
- implemented 表示实现/自测通过，accepted 表示已验收；检查点提交不代表完成。最终核对全部差异与验收，不自动安装或发布。
