...
涉及用户可见结果或界面的任务，同时读取 `references/delivery/evidence-driven-delivery.md`。正常开发以已确认需求、界面流程、原型或设计稿作为期望基线，实现后按同一路径验证，不把截图当成所有任务的统一门禁。

任务需要第三方应用、网站或桌面工具时，同时读取 `references/delivery/tooling-prerequisites.md`。进入实现前完成所选结构化方式（已有 CLI、专用连接器、官方 API 或项目脚本）的可用性、版本/能力边界和最小认证检查；只有清单明确 CLI 必需且没有等价已就绪路径时才安装。必须用户扫码、验证码、授权或系统权限时提前请求配合。
...
当前任务涉及前端页面、后台工具、审核流、任务流、表单、表格或复杂界面状态时，必须按 `tasks.md` 声明的界面基线、证据门禁和关键状态，对照已确认的 `ui-flow.md` / `prototype/` / Open Design 产物检查实现偏差。

- 不在每个任务或每次样式修改后重复截图。多个任务共用页面时，优先由一个 `QA` / `INTEGRATION` 任务集中验证。
- 首轮代码审查前只做轻量界面检查，确认主路径可达、基线可读取且无明显契约偏差；完成首轮代码审查的阻塞修复、准备复审前，再保存最终实现截图和完成原型对比，由复审同时确认修复和证据。
- 首轮代码审查零阻塞时，立即制作最终证据，优先由原审查者限定范围确认；不可恢复时由同职责且独立于实现者的审查者接替并记录原因。Goal 场景必须写入 `.goal/cr/<slice>-evidence-confirmation-<n>.md`。
- 最终证据制作后若修改任何运行时代码，必须重新做限定范围代码审查并更新证据；只有证据文件和报告变化时才允许直接进入证据确认。
- 原型对比必须检查主路径、操作矩阵、权限、状态覆盖和视觉实现，不能只做像素相似度比较。
...
- 前端任务在首次代码审查前做一次轻量界面检查；完成阻塞修复、准备复审前做最终界面偏差与证据留存。后续界面修复要复验受影响状态，并因运行时代码变化重新做限定范围代码审查。
