...
除非项目规则明确允许、用户已明确授权当前操作、且工作区干净，否则不得执行：
...
- 切分支、合并、rebase 等会迁移或改写工作区关系的操作要求工作区干净；有改动时停止，不使用 `--autostash` 绕过。`add` / `commit` 按下段的已授权检查点规则核对，不能要求待提交的改动先消失。
- 用户明确授权当前命令，而不是只说“看下 / 处理下 / 更新下”。
- 当前不在受保护主干上；如果在 `main` / `master`，只允许创建新分支或按项目 SOP 操作。

已授权的功能分支检查点：先核对本任务改动和现有暂存区，按明确路径暂存，只提交属于当前授权范围且符合实现/自测要求的内容；已有无关暂存内容时先核实，不混入提交，也不自动取消用户暂存。不能因为用户允许编辑就推定允许提交、推送或合并。已有适用授权继续有效，不为同一操作重复索要；授权范围或分支不清时才澄清。

需求 / 方案文件写入需要分支保护，但文件编辑本身不等于切分支、合并或 rebase。只在聊天里讨论不需要 Git 写操作；写入业务项目的 `requirements.md`、`plan.md`、`ui-flow.md`、`tasks.md`、`prototype/`、`.goal/*` 不得直接发生在 `main` / `master` 上。若当前在主干且工作区干净，先创建需求功能分支；若要变更分支而工作区不干净，先停止并说明风险。

已经处于本任务获授权的功能分支，且未提交改动归属已核实属于当前任务时，可以继续编辑任务文件和回写 `.goal/status.yaml`，无需先提交或重新授权。发现来源不明、范围重叠或其他任务改动时，先核实与保护它们；不要覆盖、暂存或迁移。此文件编辑规则不放宽上述 Git 命令门禁。
