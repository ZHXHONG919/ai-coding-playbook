...
项目 `.goal/status.yaml` 是 Goal Execute 的唯一执行状态源；Codex app 自带的 goal 进度条只用于 UI 可视化。用户请求创建/使用 Goal 且当前工具契约允许时，可用 `get_goal` / `create_goal` 创建或复用 app goal 镜像；不把本地规则当成突破工具调用条件的授权。
